...
public:
     ...
     */
    TcntXML(int lang = 3, const QString &name = QString())
        : m_lang(lang)
        , m_name(name)
    {
    }
...
QVariant TcountingManager::languagesModel()
{
    if (!m_languagesModel)
        m_languagesModel = new TcountLangModel(currentLanguage(), this);
    return QVariant::fromValue(m_languagesModel);
}
                ...
                if (max > noiseLevel) {
                    ...
                    if (f - onSetAt > 9000) { // 187.5 ms at least
                        ...
                        if (silCnt > 30) {
                            ...
                            //                     qDebug() << "finished" << onEndAt << (onEndAt / 48000.0) << "dur:" << (onEndAt - onSetAt) << ((onEndAt -
                            //                     onSetAt) / 48000.0);
                        }
